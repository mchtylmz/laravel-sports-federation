            ...
            <form class="js-validation-form" action="{{ route('event.save', $event->id ?? '') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-lg-7 mb-3">
                        ...
                        <input type="text" class="js-flatpickr form-control" id="start_date" name="start_date" data-locale="tr" placeholder="YYYY-AA-GG" value="{{ $event->start_date ?? now()->format('Y-m-d') }}" required>
                    </div>
                    <div class="col-lg-5 mb-3">
                        <label class="form-label" for="start_time">{{ __('events.event') }} {{ __('events.form.start_time') }}</label>
                        <input type="text" class="js-flatpickr form-control" id="start_time" name="start_time"  data-locale="tr" placeholder="SS:DD" data-enable-time="true" data-no-calendar="true" data-date-format="H:i" data-time_24hr="true" value="{{ $event->start_time ?? now()->format('H:00') }}">
                    </div>

                    <div class="col-lg-7 mb-3">
                        <label class="form-label" for="end_date">{{ __('events.event') }} {{ __('events.form.end_date') }}</label>
                        <input type="text" class="js-flatpickr form-control" id="end_date" name="end_date" data-locale="tr" placeholder="YYYY-AA-GG" value="{{ $event->end_date ?? now()->format('Y-m-d') }}" required>
                    </div>
                    <div class="col-lg-5 mb-3">
                        <label class="form-label" for="end_time">{{ __('events.event') }} {{ __('events.form.end_time') }}</label>
                        <input type="text" class="js-flatpickr form-control" id="end_time" name="end_time"  data-locale="tr" placeholder="SS:DD" data-enable-time="true" data-no-calendar="true" data-date-format="H:i" data-time_24hr="true" value="{{ $event->end_time ?? now()->format('H:i') }}">
                    </div>
                </div>
